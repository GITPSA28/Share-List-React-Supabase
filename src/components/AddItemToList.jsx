...
function AddToListContent({ item, type, custom }) {
  const { lists, isLoading } = useItemInUsersList({
    item: item.id,
    type,
    custom,
  });
...
function SelectList({ item, type, userList, isLoading }) {
  const { isUpdating, updateList } = useUpdateListItems({ item: item.id });
